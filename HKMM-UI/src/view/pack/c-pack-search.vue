        ...
        <div class="p-1 card m-2">
            <div class="input-group ">
                ...
                <button class="btn border text-white"
                    @click="db_collapse?.toggle()"><i class="bi bi-arrow-down" /></button>
            </div>
                ...
                <div v-for="(group, id) in groups" :key="id" class="row">
                    ...
                    <div v-for="(key, name) in group" :key="key" class="col form-check"> 
                        ...
                        <input :id="`cps-g-${id}-${key}`" class="form-check-input" 
                            type="checkbox" v-if="id.startsWith('mul_')"
                            :name="`cps-g-${id}`" :value="key" v-model="options"
                            @change="refresh()"/>
                        <input :id="`cps-g-${id}-${key}`" class="form-check-input" 
                            type="radio" v-else
                            :name="`cps-g-${id}`" :value="key" v-model="filters[id]"
                            @change="refresh()"/>
