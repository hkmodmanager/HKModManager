    ...
    <div>
        ...
        <div>
            <div class="d-flex" v-if="getAPIVersion() > 0 || !nofitapi">
                ...
                <button class="btn btn-primary flex-grow-1" v-if="isInstallAPI() && saveBackupFile()" @click="uninstallAPI()">{{
                        $t("api.uninstall")
                }}</button>

            </div>
            <div class="alert alert-danger" v-if="isInstallAPI() && !saveBackupFile()">{{ $t("api.noBackup") }}</div>
...
import { getAPIVersion, getGameVersion, getLatestIsMatch, getLatestIsMatchSync, downloadAPI, getBackupPath, resotreBackup } from '@/renderer/apiManager'
...
export default defineComponent({
    methods: {
        ...
        hasUpdate() {
            if (!this.apigf || !this.isInstallAPI() || !getLatestIsMatchSync()) return false;
