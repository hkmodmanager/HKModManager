var fs = require('fs')
    ...
    , builtEdge = path.resolve(__dirname, '../build/Release/' + (process.env.EDGE_USE_CORECLR || 
        !fs.existsSync(path.resolve(__dirname, '../build/Release/edge_nativeclr.node')) ? 'edge_coreclr.node' : 'edge_nativeclr.node'))
    , edge;

let rootPath = __dirname;
if (window.process.env.WEBPACK_SERVE) {
    rootPath = 'F:/HKLab/HKMM/HKMM-UI/libs/electron-edge-js-master/lib';
}
...
function checkForPreCompiled() {
    ...
    if (process.versions.electron) {
        ...
        const preCompiledPath = path.resolve(path.join(rootPath, './native/', process.platform, process.arch, version, (process.env.EDGE_USE_CORECLR ? 'edge_coreclr' : 'edge_nativeclr')));
...
if (process.env.EDGE_USE_CORECLR && !process.env.EDGE_BOOTSTRAP_DIR && fs.existsSync(path.join(rootPath, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1', 'bootstrap.dll'))) {
    process.env.EDGE_BOOTSTRAP_DIR = path.join(rootPath, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1');
}
...
exports.func = function (language, options) {
    ...
    if (typeof language !== 'string') {
        ...
        var compiler;
        try {
            compiler = n_require(compilerName);
        }
        catch (e) {
        ...
        if (process.env.EDGE_USE_CORECLR) {
            var defaultManifest = path.join(rootPath, 'bootstrap', 'bin', 'Release', 'netcoreapp1.1', 'bootstrap.deps.json');
